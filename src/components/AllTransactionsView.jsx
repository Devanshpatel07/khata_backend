...
export default function AllTransactionsView() {
  ...
  const { transactions, openTransactionModal, openEditTransactionModal, deleteTransaction } = useLedgerStore();
  ...
  const handleDelete = async (e, tx) => {
    e.stopPropagation();
    if (!window.confirm(`Are you sure you want to delete transaction of ₹${tx.amount} with ${tx.party_name}?`)) return;
    try {
  ...
  const filteredTransactions = transactions.filter(t => {
    ...
    if (searchFilter) {
      ...
      const matchParty = t.party_name ? t.party_name.toLowerCase().includes(term) : false;
  ...
  return (
    <div style={{ padding: '0 1rem 7rem 1rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div className="glass-panel" style={{ padding: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h2 style={{ fontSize: '1.4rem', fontWeight: '800', display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-main)' }}>
            <Receipt size={22} color="var(--color-cyan)" /> Transaction Ledger & Receipts
          </h2>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>
            Audit trail of all credit and debit entries across workspace parties.
          </p>
        </div>
      ...
      <div className="glass-panel" style={{ padding: '1.25rem' }}>
        {filteredTransactions.length > 0 ? (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <div>
                    <div style={{ fontSize: '0.95rem', fontWeight: '800', display: 'flex', alignItems: 'center', gap: '0.65rem', color: 'var(--text-main)' }}>
                      {tx.party_name}
                      <span className="badge badge-cyan" style={{ fontSize: '0.65rem' }}>{tx.category}</span>
                      <span className="badge badge-amber" style={{ fontSize: '0.65rem' }}>{tx.payment_mode}</span>
                    </div>

                    <div style={{ fontSize: '0.775rem', color: 'var(--text-dim)', marginTop: '0.2rem' }}>
                      {tx.notes || 'No description notes'} • {new Date(tx.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.35rem' }}>
                    <button
                      ...
                      style={{ padding: '0.4rem', borderRadius: '8px', minHeight: 'auto' }}
                      title="Edit Entry"
                    >
                      <Edit3 size={15} color="var(--color-purple)" />
                    </button>
                    <button
                      onClick={(e) => handleDelete(e, tx)}
                      className="btn-secondary"
                      style={{ padding: '0.4rem', borderRadius: '8px', minHeight: 'auto', color: 'var(--color-rose)' }}
                      title="Delete Entry"
                    >
                      <Trash2 size={15} />
                    </button>
          ...
          <div style={{ textAlign: 'center', color: 'var(--text-dim)', padding: '4rem 0' }}>
            No transactions match the selected search & filter options.
          </div>
        )}
      </div>
